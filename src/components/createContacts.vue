<template>
  ...
  <button
              @click="creatcontact({ fname: this.fname, lname: this.lname,email:this.email,cemail:this.cemail })"
...
export default {
  ...
  setup() {
    ...
    const { mutate: insert_contacts } = useMutation(
      gql`mutation insert_contacts($fname: String!, $lname: String!,$email:String!,$cemail:String!) {
       insert_contacts(objects: [{ fname: $fname, lname: $lname,email:$email,cemail:$cemail}]) {
             returning {
               fname
             }
         }
